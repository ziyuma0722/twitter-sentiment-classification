We use Google Colab to run the code.
For all linear baseline models, CPU could be used to run the code.
For all neural network models, L4 GPU/T4 GPU were used to train the model.
For all neural network models including BERT/RoBERTa, L4 was used to train the model.

To test or run the code, upload the corresponding materials in the "materials" folder into google drive and the jupyter notebook file into google colab.
For BERT/RoBERTa related files, we use transformers==4.37.2 and tensorflow==2.15.0(default for the current google colab) to ensure compatibility. Besides, for those files, submission file production code is self-contained.
...
Then the code
#checkpoint_path = '/content/drive/MyDrive/LSTM_CNN/cp-0001.ckpt'
...
Code consistency:  For the epoch number, while training we stoped when validation loss started to decrease. In the code, we kept them all the same with 30 to ensure code consistency, whereas
most of the models' peak performance were achieved within 10 epochs.
...
7. Folder "CNN": CNN models of III.B.(1) of our report
       1D CNN:
                "1D_CNN.ipynb"
       Multilayer_CNN: architecture that uses stacked convolutional layers, not mentioned in the report due to limited report length, didn't achieve good result
                "Multilayer_CNN.ipynb"
...
10. Folder "LSTM_Attention": LSTM and Attention hybrid architectures of III.C.(2) of our report
...
11. Folder "CNN_LSTM_Attention": Models using the combinations of CNN, LSTM, and Attention, III.C.(3) of our report
       Code "ABCDM": the ABCDM model
       Code "DNN_MHAT": the DNN_MHAT model

12. Folder "BERT_RoBERTa": Baseline models using BERT or BoBERTa as embeddings, part III.C.(4) of our report
...
13. Folder "NOVEL_MODELS": novel models created by ourselves, models introduced in the "IV. THE NOVEL MODEL" part of our report
